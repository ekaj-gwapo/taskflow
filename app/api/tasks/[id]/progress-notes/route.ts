  ...
  try {
    ...
    try {
      ...
      if (notifyUserIds.length > 0) {
        ...
        const notificationTitle = `New Progress Note on "${task.title}"`;
        const notificationMessage = `${auth.user!.name} added a note: "${content.substring(0, 50)}${content.length > 50 ? '...' : ''}"`;
        
        for (const userId of notifyUserIds) {
          await db.execute(
            "INSERT INTO notifications (id, userId, type, title, message, link, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)",
            [uuidv4(), userId, "NOTE_ADDED", notificationTitle, notificationMessage, `/tasks/${taskId}`, now]
          );
        }
      }
    } catch (notifError) {
      console.error("Failed to create progress note notifications:", notifError);
    }
