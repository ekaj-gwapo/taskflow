...
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedTasks = await Promise.all(tasks.map(async (t) => {
      const actionSteps = await db.getAll("SELECT * FROM action_steps WHERE taskId = ?", [t.id])
      ...
      const progressNotes = await db.getAll("SELECT * FROM progress_notes WHERE taskId = ?", [t.id])
...
export async function POST(request: NextRequest) {
  try {
    ...
    const actionStepsData = await db.getAll("SELECT * FROM action_steps WHERE taskId = ?", [taskId])
